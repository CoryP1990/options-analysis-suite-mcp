...
export function register(server: McpServer, client: ProxyClient): void {
  server.tool(
    'get_fundamentals',
    'Get company fundamentals: market cap, P/E ratio, EPS, revenue, profit margins, dividend yield, beta, sector, and industry. Useful for assessing whether an options strategy aligns with the fundamental picture. Returns TTM ratios and key metrics by default. Use full=true to include financial statements (income, balance sheet, cash flow).',
    {
      symbol: z.string().describe('Ticker symbol'),
      full: z.boolean().optional().describe('Include full financial statements (annual + quarterly). Default false — returns TTM ratios and key metrics only.'),
    },
